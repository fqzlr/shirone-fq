/**
 * 樱花特效画布管理器：优先 OffscreenCanvas + Web Worker 绘制（脱离主线程，
 * 避免页面切换时主线程阻塞导致樱花掉帧）；不支持时回退到主线程 Canvas 2D 实现。
 *
 * 本模块由 Layout.astro 的加载器在「樱花特效开启」时动态 import（零负担：
 * 关闭状态下本模块与 Worker 不进入请求队列），并监听设置面板的开关事件。
 */
import { SAKURA_TOGGLE_EVENT } from "@constants/constants.ts";
import { prefersReducedMotion } from "@utils/motion.ts";
import { getSakuraEnabled, resolveSakuraConfig } from "@utils/sakura-utils.ts";
...
class MainThreadSakuraManager implements SakuraManagerLike {
	...
	/**
	 * init/stop 周期代际标记。每次 stop 自增，使 await 期间被取消的 init 失效，
	 * 避免图片加载期间用户切换导致重复创建 canvas（DOM 泄漏）。
	 */
	private initGeneration = 0;

	constructor(config: SakuraConfig, imageUrl: string) {
		this.config = config;
	...
	async init() {
		...
		// 提前标记运行，防止 await 图片加载期间重入导致重复创建 canvas
		this.isRunning = true;
		...
		try {
			...
			// await 期间若被 stop 或被更新的 init 取代，则中止本次初始化
	...
	handleResize() {
		// 用 rAF 节流，避免 resize 事件高频触发频繁重置 canvas（重置会清空内容）
	...
	stop() {
		// 使进行中的 init 失效，避免其在 await 完成后继续创建资源
		this.initGeneration++;
...
// ===========================================================================
// Worker + OffscreenCanvas 实现（优先使用，绘制循环脱离主线程）
// ===========================================================================
class WorkerSakuraManager implements SakuraManagerLike {
	...
	private resizeRafId: number | null = null;
	/** Worker 是否已完成 init（收到 ready）。用于区分 init 阶段失败与运行期错误。 */
	...
	async init() {
		...
		try {
			// 创建 canvas 并转移到 Worker（转移后主线程不能再获取其 context）
			this.canvas = document.createElement("canvas");
			this.canvas.setAttribute(
				"style",
				`position: fixed; left: 0; top: 0; pointer-events: none; z-index: ${this.config.zIndex}; transform: translateZ(0);`,
			...
			// 创建 Worker（Vite 自动打包 src/workers/sakura.worker.ts）
			...
			this.worker.onerror = (e) => {
				...
				// init 阶段（尚未 ready）的 worker 级错误说明初始化失败，清理保持状态一致
				if (!this.workerReady) {
					this.stop();
				}
			};
			...
			this.worker.onmessage = (
				...
				if (data.type === "ready") {
					...
					// init 阶段（尚未 ready）的 error 说明初始化失败：
					// worker 内部已清理，主线程也需清理残留 canvas 与 worker，
					// 否则会留下空白 canvas + idle worker（资源泄漏），且 isRunning 状态不一致
					if (!this.workerReady) {
						this.stop();
					}
			...
			// 发送 init 消息，转移 OffscreenCanvas 所有权给 Worker
	...
	stop() {
		...
		if (this.worker) {
			// terminate 会立即回收 worker 整个 JS 堆（含 ImageBitmap），
			// postMessage({type:"stop"}) 可能因 terminate 抢先而无法处理，但无害
...
function initSakura(cfg: SakuraConfig) {
	const imageUrl = url("/images/effects/sakura.png");
	if (globalSakuraManager) {
		...
		if (supportsWorkerSakura()) {
			globalSakuraManager = new WorkerSakuraManager(cfg, imageUrl);
		} else {
			globalSakuraManager = new MainThreadSakuraManager(cfg, imageUrl);
		}
		if (cfg.enable) {
			void globalSakuraManager.init();
		}
...
/**
 * 启动樱花特效运行时：幂等（Swup 切页重跑不会重复初始化）。
 * 由 Layout.astro 加载器在特效开启（或首次开启）时动态调用，并常驻监听设置面板开关事件。
 */
export function setupSakura(): void {
	const cfg = resolveSakuraConfig();
	// 生效开关 = 访客存储偏好（加载器只在开启时调用本函数，此处兜底再读一次）
	cfg.enable = getSakuraEnabled();
	// 减少动态效果时不绘制：尊重访客动效偏好（关闭该偏好后特效开关即时生效）
	if (prefersReducedMotion()) {
		cfg.enable = false;
	}
	initSakura(cfg);

	window.addEventListener(SAKURA_TOGGLE_EVENT, (e: Event) => {
		const detail = (e as CustomEvent<{ enabled: boolean }>).detail;
		if (!detail) return;
		if (!globalSakuraManager) return;
		if (detail.enabled && !prefersReducedMotion()) {
			globalSakuraManager.config.enable = true;
			void globalSakuraManager.init();
		} else if (!detail.enabled || prefersReducedMotion()) {
			globalSakuraManager.stop();
		}
	});
}
