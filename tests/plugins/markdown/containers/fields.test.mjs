...
import {
	FieldComponent,
	FieldGroupComponent,
} from "../../../../src/plugins/markdown/containers/rehype-fields.mjs";
...
test("renders field metadata and keeps description content", () => {
	const result = FieldComponent(
		{ "has-directive-label": true, "label-optional": "Optional" },
		[
			paragraph("tex"),
...
test("renders field groups as a framed SSR container", () => {
	...
	assert.deepEqual(result.properties.className, [
		"m3-field-group",
		"not-prose",
	]);
...
test("unknown metadata lines remain readable body content", () => {
	const result = FieldComponent({ name: "value" }, [
		paragraph("@unknown keep this"),
	]);
	assert.equal(
		result.children[1].children[0].children[0].value,
		"@unknown keep this",
